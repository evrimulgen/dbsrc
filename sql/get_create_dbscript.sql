...
-- ============================================================================
-- INVOCATION	sqlplus -s user/pw@db @get_create_dbscript (set_no)
--
--			set_no is optional and defaults to max(dbsrc.set_no)
--
-- DESCRIPTION
--
-- get_create_dbscript.sql: get the "object creation" database script.
--
-- The resulting script is named in the pattern of "[cfg_name]_create.sql".
--
-- ============================================================================

prompt -- BEGIN get_create_dbscript...
...
prompt -- DESCRIPTION
prompt --;
prompt -- The intent of this script is for it to be executable by oracle, so 
prompt -- ddl is arranged in dependency order as determined by dbms_metadata. 
prompt -- As multi-cycle dependencies are possible, ORA errors encountered by 
prompt -- missing dependencies still may still occur. To address the errors,
prompt -- it should be feasible to successfully force a recompile of all 
...
select distinct
    ...
    'select ''##p_cfg_name._##p_set_no._create_''||to_char(sysdate,''YYYYMMDD_HH24MISS'')||''.log'' logfile from dual;'||chr(10)||
...
select 
    -- if post-processed, output the post-processing comment before the ddl
	-- else just output the ddl
    case when pp.order_no is not null then
...
set termout on;
prompt -- END get_create_dbscript.
