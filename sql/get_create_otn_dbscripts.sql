...
-- Create script file for each owner, ddl type and object name.
--
-- INVOCATION	sqlplus -s user/pw@db @get_create_otn_dbscripts [set_no]
--
--     		dir name: [cfg_name]
--      	script names: [cfg_name]_create.[owner].[type].[name].sql
--
--	parm 1: set_no is optional and defaults to max(dbsrc.set_no)
--
-- ============================================================================

prompt -- BEGIN get_create_otn_dbscripts ...
...
select 'out\&&p_cfg_name\create_otn' x from dual;
...
prompt -- DESCRIPTION
prompt --;
prompt -- These are database scripts for each owner, type and object name in the 
prompt -- &&p_cfg_name dbsrc configuration. DDL is arranged in dependency order 
prompt -- inside each script.
prompt --;
prompt -- Explicitly grouping ddl by owner, type and name will of course violate 
prompt -- build dependencies, so don't expect to be able to execute 
prompt -- these scripts without some dependency errors.
prompt --;
prompt -- ============================================================================
...
spool &&p_dirname\_otn_slave.sql

-- bind vars for below
prompt var o varchar2(100);
prompt var t varchar2(100);
prompt var n varchar2(100);

-- we are bursting into possibly VERY many scripts here by
--  carefully varying the spool files we write to:
--      create.[owner].[object_type].[object_name].sql
...
select
    'spool &&p_dirname\create_'||
		...
		case when object_name = 'NO_NAME' then 'x' else lower(object_name) end||
		'.sql'||
--
-- now get the ddl
'
...
--
group by object_owner, ddl_type, object_name
order by object_owner, ddl_type, object_name;
spool off;
...
set termout on
prompt --   Creating &&p_dirname\[cfg_name]_create.[owner].[type].[name].sql scripts
set termout off

@@&&p_dirname\_otn_slave.sql

-- =============================================================================
-- (4) cleanup
-- =============================================================================

host del "&&p_dirname\_otn_slave.sql"

set termout on
prompt -- END get_create_otn_dbscripts.
