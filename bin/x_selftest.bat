...
:: export database source then generates database script from that source
call bin/_export_ddl selftest
if %errorlevel% neq 0 goto error
call %dbsrc_home%/bin/_get_dbscript
if %errorlevel% neq 0 goto error
...
if %errorlevel% neq 0 goto error
call sqlplus -s %dbsrc_user%/%dbsrc_pw%@%dbsrc_db% @sql/get_create_otn_dbscripts
if %errorlevel% neq 0 goto error
goto end
