...
:: simple wrapper for sql scripts to simplify invocation
:: requires a single parameter: a dbsrc_cfg.cfg_name
:: see individual sql scripts for more info.
call sqlplus -s %dbsrc_user%/%dbsrc_pw%@%dbsrc_db% @sql/get_def_dbscript
if %errorlevel% neq 0 goto error
call sqlplus -s %dbsrc_user%/%dbsrc_pw%@%dbsrc_db% @sql/get_manifest
if %errorlevel% neq 0 goto error
call sqlplus -s %dbsrc_user%/%dbsrc_pw%@%dbsrc_db% @sql/get_drop_dbscript
if %errorlevel% neq 0 goto error
call sqlplus -s %dbsrc_user%/%dbsrc_pw%@%dbsrc_db% @sql/get_create_dbscript
if %errorlevel% neq 0 goto error
call sqlplus -s %dbsrc_user%/%dbsrc_pw%@%dbsrc_db% @sql/get_create_ordered_dbscript
if %errorlevel% neq 0 goto error
goto end
