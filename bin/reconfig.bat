...
:: execute all sql scripts in the config directory
for /F %%x in ('dir /B %dbsrc_home%\cfg\*.sql') do (
	set filename=%config_dir%\%%x
	call sqlplus -s %dbsrc_user%/%dbsrc_pw%@%dbsrc_db% @cfg/!filename!
