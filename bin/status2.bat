...
:: report a brief status summarizing the contents of the dbsrc table for the
:: current (most recent) set of dbsrc records.
( echo @@sql/_init_interactive echo exec dbsrc_mgr.desc_dbsrc_set; echo exit;)
| call sqlplus -s %dbsrc_user%/%dbsrc_pw%@%dbsrc_db% goto end
